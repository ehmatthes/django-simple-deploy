...
def call_simple_deploy(tmp_dir, sd_command, platform=None):
    ...
    print(f"Python interpreter path: {python_exe}")
    print(f"Final command: {sd_command}")

    # Make the call to simple_deploy.
    #   The `text=True` argument causes this to return stdout and stderr as strings, not objects.
    sd_call = subprocess.Popen(split(sd_command), stdout=subprocess.PIPE, stderr=subprocess.PIPE, text=True, cwd=tmp_dir)
